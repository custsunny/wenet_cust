...
5. **File preparation**
   ...
   **Filespec：**
   ...
   - [ ] **`model_dir`**
    ...
    ' Final.zip' and 'units.txt' are required files for speech recognition in libtorch runtime environment，`final.pt`、`global_cmvn`、`train.yaml`和`units.txt`是模型继续训练/参数微调的必须文件。
